"""
Script 4: Animación con refugio circular en el centro
...
def crear_animacion_refugio_circular():
    """
    Crea animación con refugio circular en el centro.
    """
    print("\n" + "="*70)
    print("GENERANDO ANIMACIÓN: REFUGIO CIRCULAR EN EL CENTRO")
    print("="*70)
    ...
    # Generar refugio circular en el centro
    print("\n2. Generando refugio circular en el centro...")
    gen = RefugeGenerator(L=L, nx=nx, ny=ny)

    # Círculo central con radio 2.0
    refuge_map = gen.generate_circular_refuges(
        centers=[(L/2, L/2)],
        radii=[2.0],
        enhancement=2.5
    )
    print(f"   ✓ Refugio circular creado (centro: ({L/2}, {L/2}), radio: 2.0)")
    ...
    # Dibujar círculo del refugio
    circle = plt.Circle((L/2, L/2), 2.0, color='green', fill=False,
                       linestyle='--', linewidth=2, label='Refugio')
    ...
    ax_spatial.set_title('Refugio Circular Central - Spawn Aleatorio\nt = 0.00 s', fontsize=14, fontweight='bold')
    ...
    def update(frame):
        ...
        # Actualizar título
        ax_spatial.set_title(f'Refugio Circular Central - Spawn Aleatorio\nt = {times[frame]:.2f} s',
    ...
    output_path = output_dir / '4_refugio_circular_centro.gif'
...
if __name__ == "__main__":
    crear_animacion_refugio_circular()
